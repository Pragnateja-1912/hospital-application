...
export default function ManageAppointments() {
    ...
    async function handleConfirm(id) {
        axios.put(`https://hospital-application-flpg.onrender.com/api/appointments/confirm/${id}`)
            .then((res) => {
                if (res.status === 204)
                    alert("Confirmed")
            })
    ...
    async function handleCancel(id) {
        axios.put(`https://hospital-application-flpg.onrender.com/api/appointments/cancel/${id}`)
            .then((res) => {
                if (res.status === 204)
                    alert("Cancelled")
            })
    ...
    async function fetchAppointments() {
        axios.get("https://hospital-application-flpg.onrender.com/api/appointments")
            .then((res) => {
                if (res.status === 200) {
